...
//  Project20
//
//  Created by Ceren Güneş on 17.12.2023.
//
...
class GameScene: SKScene {
    ...
    func explode(firework: SKNode) {
        if let emitter = SKEmitterNode(fileNamed: "explode") {
            ...
            let waitAction = SKAction.wait(forDuration: TimeInterval(emitter.numParticlesToEmit) * emitter.particleLifetime)
    ...
    func explodeFireworks() {
        ...
        for (index, firewoworkContainer) in fireworks.enumerated().reversed() {
            guard let firework = firewoworkContainer.children.first as? SKSpriteNode else { continue }
            
            if firework.name == "selected" {
                explode(firework: firewoworkContainer)
                fireworks.remove(at: index)
                numExploded += 1
            }
        }
